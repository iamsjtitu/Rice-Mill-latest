...
async def export_excel(
    ...
    # ===== CASH TRANSACTIONS SECTION (Separate Sheet) =====
    ct_query = {}
    if kms_year: ct_query["kms_year"] = kms_year
    if season: ct_query["season"] = season
    cash_txns = await db.cash_transactions.find(ct_query, {"_id": 0}).sort("date", -1).to_list(10000)
    
    if cash_txns:
        ws2 = wb.create_sheet(title="Cash Transactions")
        
        # Title
        ws2.merge_cells('A1:F1')
        ws2['A1'] = f"Cash Transactions | KMS: {kms_year or 'All'} | {season or 'All Seasons'}"
        ...
        ws2['A2'] = f"Generated: {datetime.now().strftime('%d-%m-%Y %H:%M')}"
        ws2['A2'].alignment = center_align
        
        ct_headers = ["Date", "Party Name", "Type (Jama/Nikasi)", "Amount (Rs.)", "Description", "Payment Mode"]
        ...
        for t in cash_txns:
            ...
            for col, value in enumerate(row_data, 1):
                cell = ws2.cell(row=ct_row, column=col, value=value)
                cell.border = thin_border
                if col == 4:
                    cell.alignment = right_align
        ...
        # Totals
        for col, value in enumerate(["TOTAL", "", "Jama:", round(total_jama, 2), "", ""], 1):
            cell = ws2.cell(row=ct_row, column=col, value=value)
            cell.fill = total_fill
            cell.font = total_font
            cell.border = thin_border
        ct_row += 1
        for col, value in enumerate(["", "", "Nikasi:", round(total_nikasi, 2), "", ""], 1):
            cell = ws2.cell(row=ct_row, column=col, value=value)
            cell.fill = total_fill
            cell.font = total_font
            cell.border = thin_border
        ct_row += 1
        for col, value in enumerate(["", "", "Balance:", round(total_jama - total_nikasi, 2), "", ""], 1):
            cell = ws2.cell(row=ct_row, column=col, value=value)
            cell.fill = PatternFill(start_color="FDE68A", end_color="FDE68A", fill_type="solid")
            cell.font = Font(bold=True, size=10)
            cell.border = thin_border
        
        ct_col_widths = [12, 18, 16, 14, 40, 12]
        for i, width in enumerate(ct_col_widths, 1):
...
async def export_pdf(
    ...
    if cash_txns:
        ...
        ct_title_style = ParagraphStyle('CTTitle', parent=styles['Heading1'], fontSize=12, textColor=colors.white, alignment=TA_CENTER, spaceAfter=2*mm)
        ct_title_data = [[Paragraph("<b>Cash Transactions / लेन-देन</b>", ct_title_style)]]
        ct_title_table = Table(ct_title_data, colWidths=[page_width - 16*mm])
        ct_title_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#1E293B')),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('TOPPADDING', (0, 0), (-1, -1), 3),
            ('BOTTOMPADDING', (0, 0), (-1, -1), 3),
        ]))
        elements.append(ct_title_table)
        elements.append(Spacer(1, 2*mm))
        
        ct_headers = ["Date", "Party Name", "Type", "Amount (Rs.)", "Description", "Payment Mode"]
        ct_data = [ct_headers]
        total_jama = 0
        total_nikasi = 0
        ...
        ct_data.append(["TOTAL", "", "Jama", f"{total_jama:,.2f}", "", ""])
        ct_data.append(["", "", "Nikasi", f"{total_nikasi:,.2f}", "", ""])
        ct_data.append(["", "", "Balance", f"{(total_jama - total_nikasi):,.2f}", "", ""])
        
        ct_col_widths = [14*mm, 24*mm, 12*mm, 18*mm, 38*mm, 14*mm]
        ct_table = Table(ct_data, colWidths=ct_col_widths, repeatRows=1)
        ...
        for i in range(1, len(ct_data) - 3):
            if i % 2 == 0:
                ct_style.append(('BACKGROUND', (0, i), (-1, i), alt_row_bg))
        
        ct_table.setStyle(TableStyle(ct_style))
