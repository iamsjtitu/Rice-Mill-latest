...
class TestPDFContent:
    ...
    def test_pdf_contains_navkar_agro(self):
        """Verify PDF contains NAVKAR AGRO text"""
        response = requests.get(f"{BASE_URL}/api/export/pdf")
        
        # Check if NAVKAR AGRO appears in PDF content
        # Note: PDF text is encoded, so we check for partial matches
        content_str = response.content.decode('latin-1', errors='ignore')
        assert "NAVKAR" in content_str or "Navkar" in content_str, "PDF should contain NAVKAR AGRO"
        print("✓ PDF contains NAVKAR AGRO text")
