...
export const DCEntries = ({ filters, user }) => {
  ...
  const openEditDepot = (dc) => {
    setEditDepot(dc);
    setEditDepotForm({
      depot_name: dc.depot_name || '',
      depot_code: dc.depot_code || '',
      delivery_to: dc.delivery_to || 'FCI',
      no_of_lots: dc.no_of_lots || ''
    });
  };

  const handleSaveEditDepot = async () => {
    if (!editDepot) return;
    try {
      await axios.put(`${API}/dc-entries/${editDepot.id}`, { ...editDepot, ...editDepotForm });
      toast.success("Depot info updated!");
      setEditDepot(null);
      fetchData();
    } catch (e) { toast.error("Update failed: " + (e.response?.data?.detail || e.message)); }
  };
  ...
  // Build delivery field suggestions from historical deliveries (across all DCs)
  // vehicle_no/driver_name/slip_no are stored as "T1 / T2" (slash-joined) so we split them back
  const allDeliveries = useMemo(() => (dcs || []).flatMap(d => d.deliveries || []), [dcs]);
  const splitJoined = (s) => (s || '').split('/').map(x => x.trim()).filter(Boolean);
  const vehicleSuggestions = useMemo(() => [...new Set(allDeliveries.flatMap(d => splitJoined(d.vehicle_no)))].sort(), [allDeliveries]);
    ...
    <div className="space-y-3" data-testid="dc-entries-tab">
      ...
      <Card className="bg-slate-800 border-slate-700"><CardContent className="p-0"><div className="overflow-x-auto">
        <Table className="w-full table-auto"><TableHeader><TableRow className="border-slate-700 hover:bg-transparent">
          ...
          <TableHead className="text-slate-300 text-xs">To</TableHead>
          <TableHead className="text-slate-300 text-xs">Depot Name</TableHead>
          ...
          <TableHead className="text-slate-300 text-xs w-16 text-center">Action</TableHead>
        ...
        <TableBody>
          ...
          : filteredDCs.map(dc => (<React.Fragment key={dc.id}>
            <TableRow key={dc.id} className="border-slate-700 cursor-pointer hover:bg-slate-750" onClick={() => handleExpandDC(dc.id)} data-testid={`dc-row-${dc.id}`}>
              ...
              <TableCell className="w-16 px-2" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-center gap-0.5">
                  <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-sky-400 hover:text-sky-300 hover:bg-sky-900/30" onClick={() => openEditDepot(dc)} title="Edit Depot Info" data-testid={`dc-edit-${dc.id}`}><Edit className="w-3 h-3" /></Button>
                  {user.role === 'admin' && <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-red-400" onClick={() => handleDeleteDC(dc.id)} data-testid={`dc-delete-${dc.id}`}><Trash2 className="w-3 h-3" /></Button>}
                </div>
              </TableCell>
            </TableRow>
            {expandedDC === dc.id && (
      ...
      {/* Edit Depot Info Dialog (Quick fix for legacy DCs missing depot info) */}
      <Dialog open={!!editDepot} onOpenChange={v => { if (!v) setEditDepot(null); }}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-md" data-testid="edit-depot-dialog">
          <DialogHeader><DialogTitle className="text-sky-400">Edit Depot Info — {editDepot?.dc_number}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-slate-400">Delivery To</Label>
                <Select value={editDepotForm.delivery_to} onValueChange={v => setEditDepotForm(p => ({...p, delivery_to: v}))}>
                  <SelectTrigger className="bg-slate-700 border-slate-600 text-white h-8 text-sm" data-testid="edit-depot-delivery-to"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="FCI">FCI</SelectItem><SelectItem value="RRC">RRC</SelectItem></SelectContent>
                </Select></div>
              <div><Label className="text-xs text-slate-400">No. of Lots</Label>
                <Input type="number" value={editDepotForm.no_of_lots} onChange={e => setEditDepotForm(p => ({...p, no_of_lots: e.target.value}))} className="bg-slate-700 border-slate-600 text-white h-8 text-sm" data-testid="edit-depot-lots" /></div>
            </div>
            <div className="flex gap-2 pt-2">
              <Button onClick={handleSaveEditDepot} className="bg-sky-600 hover:bg-sky-500 text-white flex-1" data-testid="edit-depot-save">Save</Button>
              <Button variant="outline" className="border-slate-600 text-slate-300" onClick={() => setEditDepot(null)}>Cancel</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-slate-400">Delivery To</Label>
                <Select value={form.delivery_to} onValueChange={v => setForm(p=>({...p,delivery_to:v}))}>
                  <SelectTrigger className="bg-slate-700 border-slate-600 text-white h-8 text-sm" data-testid="dc-form-delivery-to"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="FCI">FCI</SelectItem><SelectItem value="RRC">RRC</SelectItem></SelectContent>
                </Select></div>
              <div><Label className="text-xs text-slate-400">No. of Lots</Label>
                <Input type="number" value={form.no_of_lots} onChange={e => setForm(p=>({...p,no_of_lots:e.target.value}))} className="bg-slate-700 border-slate-600 text-white h-8 text-sm" data-testid="dc-form-lots" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-slate-400">Depot Name</Label>
          ...
          <form onSubmit={handleAddDelivery} className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><Label className="text-xs text-slate-400">Contract No</Label>
                <Input value={delForm.contract_no} onChange={e => setDelForm(p=>({...p,contract_no:e.target.value}))} className="bg-slate-700 border-slate-600 text-white h-8 text-sm" data-testid="delivery-form-contract" autoComplete="off" /></div>
            </div>
            ...
            <div className="bg-slate-900/50 border border-slate-700 rounded-md p-2.5 space-y-2">
                ...
                <div key={idx} className="bg-slate-800 border border-slate-600 rounded p-2 space-y-1.5">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div><Label className="text-[10px] text-slate-400">Vehicle No</Label>
                      <Input list="del-vehicle-list" value={truck.vehicle_no} onChange={e => setDelForm(p => ({ ...p, trucks: p.trucks.map((t, i) => i === idx ? { ...t, vehicle_no: e.target.value } : t) }))} className="bg-slate-700 border-slate-600 text-white h-7 text-xs" data-testid={`delivery-truck-${idx}-vehicle`} autoComplete="off" /></div>
                    <div><Label className="text-[10px] text-slate-400">Driver Name</Label>
                      <Input list="del-driver-list" value={truck.driver_name} onChange={e => setDelForm(p => ({ ...p, trucks: p.trucks.map((t, i) => i === idx ? { ...t, driver_name: e.target.value } : t) }))} className="bg-slate-700 border-slate-600 text-white h-7 text-xs" data-testid={`delivery-truck-${idx}-driver`} autoComplete="off" /></div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><Label className="text-[10px] text-amber-400 font-semibold">Bags (Govt)</Label>
                      <Input list="del-bags-list" type="number" value={truck.bags_used} onChange={e => setDelForm(p => ({ ...p, trucks: p.trucks.map((t, i) => i === idx ? { ...t, bags_used: e.target.value } : t) }))} placeholder="0" className="bg-slate-700 border-slate-600 text-white h-7 text-xs" data-testid={`delivery-truck-${idx}-bags`} autoComplete="off" /></div>
              ...
              <datalist id="del-vehicle-list">{vehicleSuggestions.map(v => <option key={v} value={v} />)}</datalist>
              <datalist id="del-driver-list">{driverSuggestions.map(v => <option key={v} value={v} />)}</datalist>
              <datalist id="del-bags-list">{bagsSuggestions.map(v => <option key={v} value={v} />)}</datalist>
