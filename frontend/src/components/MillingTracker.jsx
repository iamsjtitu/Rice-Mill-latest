...
const MillingEntriesTab = ({ filters, user, paddyStock, frkStock, onRefresh }) => {
  ...
  const exportReport = async (format) => {
    try {
      ...
      if (filters.kms_year) params.append('kms_year', filters.kms_year);
      if (filters.season) params.append('season', filters.season);
      const res = await axios.get(`${API}/milling-report/${format}?${params.toString()}`, { responseType: 'blob' });
        ...
        {paddyStock && (
          <Card className="bg-gradient-to-r from-slate-800 to-slate-800/80 border-slate-700">
            <CardContent className="p-3">
              <p className="text-[10px] text-slate-400 mb-1">Paddy Stock (Mill W.)</p>
        ...
        <TableBody>
          ...
          : entries.map(e => (
            <TableRow key={e.id} className="border-slate-700">
              ...
              <TableCell className="text-xs"><span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${e.rice_type === 'parboiled' ? 'bg-amber-500/20 text-amber-400' : 'bg-cyan-500/20 text-cyan-400'}`}>{e.rice_type === 'parboiled' ? 'Usna' : 'Arwa'}</span></TableCell>
...
// ===== Sub-tab: Paddy Custody Register =====
const PaddyCustodyTab = ({ filters }) => {
  ...
  const exportExcel = async () => {
    try {
      ...
      if (filters.kms_year) params.append('kms_year', filters.kms_year);
      if (filters.season) params.append('season', filters.season);
      const res = await axios.get(`${API}/paddy-custody-register/excel?${params}`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement('a'); a.href = url; a.download = 'paddy_custody_register.xlsx'; a.click();
      ...
      {register && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card className="bg-slate-800 border-slate-700"><CardContent className="p-3 text-center">
            ...
            <p className={`text-xl font-bold ${register.final_balance >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{register.final_balance} Q</p>
          </CardContent></Card>
        </div>
      )}
      <div className="flex gap-2">
        ...
        <TableBody>
          ...
          : register.rows.map((r, i) => (
            <TableRow key={i} className={`border-slate-700 ${r.type === 'received' ? 'bg-green-900/5' : 'bg-red-900/5'}`}>
              ...
              <TableCell className="text-white text-xs text-right font-bold">{r.balance_qntl}</TableCell>
            </TableRow>))}
        </TableBody></Table>
      </div></CardContent></Card>
    </div>
