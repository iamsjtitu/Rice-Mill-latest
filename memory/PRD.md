...
- **All Payment Sections → Ledger Source of Truth**: Fixed Truck, Agent, Diesel, Local Party, and Truck Owner payment sections to calculate paid_amount from the ledger instead of their individual payment collections. Manual Cash Book payments now correctly reflect across ALL payment pages with proper status (paid/partial/pending) and payment history.
