...
- **v104.26.1 (Feb 2026)** [HOTFIX]: electron-builder `files[]` mein `license-manager.js`, `cloudflared-manager.js`, aur `activation-ui/**/*` missing the — build ke baad "Cannot find module './license-manager'" crash aa raha tha. Fix: teeno ko explicitly whitelist kiya. Patch version bump v104.26.0 → v104.26.1 taaki new build trigger ho.
- **v104.26.0 (Feb 2026)**: Cloud Access feature Desktop App ke andar. Settings → License tab mein naya section — one-click "Enable Cloud Access" button jo Cloudflared.exe auto-download (from GitHub Releases), Windows service install, tunnel connect sab karta hai. Central Server: `POST /api/license/provision-cloud-access` (license-authenticated, not super-admin — customer PC se trigger hota hai), `GET /api/license/cloud-access-status/:key`. Admin panel: `POST /api/admin/licenses/:id/mark-external-tunnel` for legacy customers with manually-configured tunnels (like aapka `mill.9x.design`). Safety: pre-existing cloudflared service auto-detect, UI shows "Pre-Configured" state instead of auto-replacing. Five UI states: fresh, installing-with-progress, live, paused, pre-existing.
