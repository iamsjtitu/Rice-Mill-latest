...
router.get('/api/export/pdf', (req, res) => {
  try {
    ...
    const headers = ['Date','Truck','Agent','Mandi','QNTL','BAG','G.Dep','GBW','P.Pkt','P.Cut','Mill W','M%','M.Cut','C%','D/D/P','Final W','G.Iss','Cash','Diesel'];
    const rows = entries.map(e => [
      e.date || '', e.truck_no || '', e.agent_name || '', e.mandi_name || '',
      (e.qntl || 0).toFixed(2), e.bag || 0, e.g_deposite || 0,
      ((e.gbw_cut || 0) / 100).toFixed(2), e.plastic_bag || 0,
      ((e.p_pkt_cut || 0) / 100).toFixed(2), ((e.mill_w || 0) / 100).toFixed(2),
      e.moisture || 0, ((e.moisture_cut || 0) / 100).toFixed(2),
      e.cutting_percent || 0, e.disc_dust_poll || 0,
      ((e.final_w || 0) / 100).toFixed(2), e.g_issued || 0,
      e.cash_paid || 0, e.diesel_paid || 0
    ]);
    const colWidths = [38,38,38,38,32,24,24,28,24,28,34,22,28,22,24,34,26,30,30];
