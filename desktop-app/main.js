...
class JsonDatabase {
    ...
    if (cashPaid > 0) {
      ...
      if (truckNo) {
        this.data.cash_transactions.push({
          ...
          amount: roundAmount(cashPaid), reference: `truck_cash_ded:${newEntry.id.slice(0,8)}`,
          kms_year: newEntry.kms_year||'', season: newEntry.season||'',
          created_by: newEntry.created_by||'system', linked_entry_id: newEntry.id,
          created_at: now, updated_at: now
        });
      ...
      if (cashPaid > 0) {
        ...
        if (truckNo) {
          this.data.cash_transactions.push({
            ...
            party_type: 'Truck',
            description: `Truck Cash Advance: ${truckNo} - Rs.${cashPaid}`,
            amount: roundAmount(cashPaid), reference: `truck_cash_ded:${id.slice(0,8)}`,
            kms_year: updated.kms_year||'', season: updated.season||'',
            created_by: updated.created_by||'system', linked_entry_id: id,
            created_at: now, updated_at: now
          });
  ...
  // Truck Payments
  // Get the per-mandi default bhada rate from a matching Mandi Target row.
...
function createApiServer(database) {
  ...
  apiApp.get('/api/lan-clients', safeSync((req, res) => {
  ...
  const routeDefs = [
    ...
    { name: 'vehicle_weight', load: () => require('./routes/vehicle_weight')(database) },
    ...
    { name: 'license', load: () => require('./routes/license')(database) },
    { name: 'letter_pad', load: () => require('./routes/letter_pad')(database) },
