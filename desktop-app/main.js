...
// Use professional PDF helpers
const { addPdfHeader: _addPdfHeader, addPdfTable, addSummaryBox, addTotalsRow, addSectionTitle, fmtAmt: _fmtAmt, C: _C } = require('./routes/pdf_helpers');
function addPdfHeader(doc, title) {
  const branding = db ? db.getBranding() : { company_name: 'Mill Entry System', tagline: '' };
...
function createApiServer(database) {
  ...
  apiApp.get('/api/diesel-accounts/pdf', safeSync((req, res) => {
    let txns = database.data.diesel_accounts || [];
    if (req.query.kms_year) txns = txns.filter(t => t.kms_year === req.query.kms_year);
    if (req.query.season) txns = txns.filter(t => t.season === req.query.season);
    const pumps = database.data.diesel_pumps || [];
