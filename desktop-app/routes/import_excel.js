...
function calculateAutoFields(data) {
  ...
  data.moisture_cut = +(moistureCutQntl * 100).toFixed(2);
  ...
  data.cutting = +(cuttingQntl * 100).toFixed(2);
...
router.post('/api/entries/import-excel', upload.single('file'), safeAsync(async (req, res) => {
  ...
  if (!headerRow) return res.status(400).json({ detail: "Header row nahi mila." });
