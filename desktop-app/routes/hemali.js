...
module.exports = (database) => {
  ...
  router.get('/api/hemali/export/pdf', safeHandler(async (req, res) => {
    ...
    const summaryH = 26;
    // Dark navy strip with gradient-like double border
    doc.rect(startX, y, tableW, summaryH).fill('#1e293b');
    doc.rect(startX, y, tableW, 2).fill('#f59e0b'); // gold accent stripe at top

    const fmtRs = (n) => `Rs.${(n || 0).toFixed(2)}`;
    // Build inline pill-style stats with separators
    const stats = [
    ...
    // Each stat takes equal width
    const cellW = tableW / stats.length;
    stats.forEach((s, i) => {
      const cx = startX + i * cellW;
      // separator line (skip before first)
  ...
  router.get('/api/hemali/export/excel', safeHandler(async (req, res) => {
    ...
    // ===== Beautiful single-line summary banner (below the totals row) =====
